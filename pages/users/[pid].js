	...
	return (
		<Layout>
			<div className="container my-4">
				<MayBeSpinner test={singleUser.is_loading} spinner={<LoadingSpinner />}>
					<MayBeSpinner test={isEmpty(singleUser.user)} spinner={<Empty />}>
						<Breadcrumb
							items={[
								{
									title: 'Home',
						...
						<div className="row">
							<div className="col-lg-3 mb-4">
								<div className="text-center bg-light rounded-lg shadow-sm mb-4 p-4">
									...
									/>
									<Maybe
										test={currentUser.is_authenticated && singleUser.user?.user_name === currentUser.user?.user_name}
									>
										<h6>Upload a different photo...</h6>
										<CustomLink
											className="btn btn-primary"
											href="/users/[pid]/edit"
											as={`/users/${singleUser.user?.user_name}/edit`}
										>
											Edit profile
										</CustomLink>
									</Maybe>
									<div>
										<Maybe test={currentUser.is_authenticated}>
											<FollowUserButton
												isUser={singleUser.user?.user_name === currentUser.user?.user_name}
